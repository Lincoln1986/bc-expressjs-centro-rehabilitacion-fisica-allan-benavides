...
// Mockeamos el repositorio de usuarios para aislar la lógica de negocio.
// ============================================
...
>;
const mockFindByIdWithTokens = usersRepo.findByIdWithTokens as jest.MockedFunction<
  typeof usersRepo.findByIdWithTokens
>;
...
describe('AuthService', () => {
  ...
  describe('register', () => {
    ...
    it('should hash the password before creating', async () => {
      ...
      // Assert — verificar que se llamó create con contraseña hasheada
  ...
  describe('login', () => {
    ...
    it('should return tokens when credentials are valid', async () => {
      // Arrange
      const hashedPassword = await bcrypt.hash(loginDto.password, 1); // rounds=1 para velocidad
    ...
    it('should throw AppError(401) when password is wrong', async () => {
      // Arrange — usuario existe pero con contraseña diferente
    ...
    it('should store hashed refresh token in database', async () => {
      ...
      // Assert — verificar que se almacenó un hash, no el token en claro
      ...
      expect(storedHash).toMatch(/^\$2[aby]?\$/); // hash de bcrypt
    });
  });
