...
//
// Tests de integración para las rutas de pacientes.
...
describe('Patient Routes — Integration Tests', () => {
  ...
  describe('GET /api/v1/patients/:id', () => {
    it('should return 200 with patient data', async () => {
      // Arrange — crear usuario y paciente
      userCookies = await registerAndLogin({
        name: 'Alice',
        email: 'alice@test.com',
        password: 'Password1!',
  ...
  describe('DELETE /api/v1/patients/:id', () => {
    it('should return 204 when admin deletes patient', async () => {
      // Arrange — crear usuario regular y admin
      userCookies = await registerAndLogin({
        name: 'Alice',
        email: 'alice@test.com',
        password: 'Password1!',
      ...
      // Crear paciente con usuario regular
      const createRes = await request(app)
        .post('/api/v1/patients')
        .set('Cookie', userCookies)
        .send({
      ...
      // Crear admin (directamente en DB para testing)
      const adminData = {
        name: 'Admin',
    ...
    it('should return 403 when regular user tries to delete', async () => {
      ...
      // Act — usuario regular intenta eliminar
      const res = await request(app)
        .delete(`/api/v1/patients/${patientId}`)
        .set('Cookie', userCookies);
